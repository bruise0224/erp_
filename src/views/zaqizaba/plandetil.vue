<template>
    <div class="container-part"> <!--//HTML代码、定义页面的区域-->
      <el-form id="form1" class="form-part" :inline="true" :model="formData1"><!--//定义表ID，表格样式和数据对象，Vue绑定指令-->
        <el-form-item label="详细行程管理---Day1(2025-1-6)" label-width="500"><!--//定义表中的标签，标签的宽度（像素）-->
        </el-form-item>
        <el-form-item>
        <el-button type="primary" @click="addData">添加行程</el-button>
        </el-form-item>
      </el-form>
      ...
      <el-table id="table1" class="table-part" :data="tableData1" border>
        ...
        <el-table-column fixed="right" label="操作" width="120"><!--//表格列始终固定在右侧-->
          <template #default="{ row }">
            <el-button link type="primary" size="small" @click="editHandle(row)">修改行程</el-button>
      ...
      <el-dialog v-model="dialogVisible" :title="dialogTitle">
        ...
        <template #footer><!--//具名插槽，定义对话框底部的信息-->
          <span class="dialog-footer">
            ...
            <el-button type="primary" @click="dialogCommit">确定修改</el-button>
          </span>
        </template>
      </el-dialog>
  ...
  const showCols = ref(['shijian', 'xingzhi', 'neirong', 'tel',])
  //prop代表属性名，isShow为布尔值，用于表示初始状态下是否显示
  //数据列  //20231215★★★
  const colOptions = ref([
    { prop: 'shijian', label: '时间', isShow: true, width: '', formatter: '' },
    { prop: 'xingzhi', label: '性质', isShow: true, width: '', formatter: '' },
    { prop: 'neirong', label: '内容', isShow: true, width: '', formatter: '' },
  ...
  const colList = ref(
    [
      { prop: 'shijian', label: '时间' },
      { prop: 'xingzhi', label: '性质' },
      { prop: 'neirong', label: '内容' },
      { prop: 'tel', label: '电话' },
