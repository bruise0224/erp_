<template>
  <div class="container-part"> <!--//HTML代码、定义页面的区域-->
    <el-form id="form1" class="form-part" :inline="true" :model="formData1"><!--//定义表ID，表格样式和数据对象，Vue绑定指令-->
      <router-link to="/Change/base_infor"><button>更新团基本信息</button></router-link>
      <router-link to="/Change/plan_manage"><button>行程管理</button></router-link>
      <router-link to="/Change/people_list"><button>游客名单</button></router-link>
      <router-link to="/Change/order_manage"><button>预定管理</button></router-link>
      <router-link to="/Change/message_print"><button>信息打印</button></router-link>
    </el-form>
    <el-form id="form2" class="form-part1" :inline="true" :model="formData1"><!--//定义表ID，表格样式和数据对象，Vue绑定指令-->
      WELCOME
      欢迎:
      GROUP NAME:241201-D-FCE1251
      ADS CODE:
      ADS CHINA:Beijing Universe Flying International
      Travel Service Co.,label-width
      ADS AUSTRALIA:Pioneer Travel Australia Pty Ltd
      No.Pax:0
    </el-form>
