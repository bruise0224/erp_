<template>
      ...
      <el-table id="table1" class="table-part" :data="tableData1" border>
        ...
        <el-table-column fixed="right" label="操作" width="120"><!--//表格列始终固定在右侧-->
          <template #default="{ row }">
            ...
            <router-link to="/zaqizaba/plandetil"><button>日程管理</button></router-link>
            <router-link to="/zaqizaba/jiudianadd"><button>酒店管理</button></router-link>
          </template>
        </el-table-column>
  ...
  const showCols = ref(['Tour_Day', 'Citys', 'Fight_No', 'Hotel',])
  //prop代表属性名，isShow为布尔值，用于表示初始状态下是否显示
  //数据列  //20231215★★★
  const colOptions = ref([
    { prop: 'Tour_Day', label: 'Tour Day', isShow: true, width: '', formatter: '' },
    { prop: 'Citys', label: 'Citys', isShow: true, width: '', formatter: '' },
    { prop: 'Fight_No', label: 'Fight No', isShow: true, width: '', formatter: '' },
    { prop: 'Hotel', label: 'Hotel', isShow: true, width: '', formatter: '' },
  ])
  ...
  const colList = ref(
    [
      { prop: 'Flight_No', label: 'Flight_No' },
    ]
  )
  //响应式引用
