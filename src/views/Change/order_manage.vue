<template>
    <div class="container-part"> <!--//HTML代码、定义页面的区域-->
      <el-form id="form1" class="form-part" :inline="true" :model="formData1"><!--//定义表ID，表格样式和数据对象，Vue绑定指令-->
        <el-form-item label="预定管理" label-width="70"><!--//定义表中的标签，标签的宽度（像素）-->
            
        </el-form-item>
  
        <el-form-item>
          <el-button >批量更新确认码</el-button>
        </el-form-item><!--//按钮功能，定义按钮ID和样式，@click表示事件绑定语法-->
        <router-link to="/zaqizaba/yudingadd"><button>酒店修改&打印</button></router-link>
      </el-form>
  ...
  const showCols = ref(['leixing', 'mubiao', 'arrive_date', 'departure_date', 'querenma', 'zongji',])
  //prop代表属性名，isShow为布尔值，用于表示初始状态下是否显示
  //数据列  //20231215★★★
  const colOptions = ref([
    { prop: 'leixing', label: '类型', isShow: true, width: '', formatter: '' },
    { prop: 'mubiao', label: '目标', isShow: true, width: '', formatter: '' },
    { prop: 'arrive_date', label: '到达日期', isShow: true, width: '', formatter: '' },
    { prop: 'departure_date', label: '离开日期', isShow: true, width: '170', formatter: '' },
    { prop: 'querenma', label: '确认码', isShow: true, width: '', formatter: '' },
    { prop: 'zongjia', label: '总价', isShow: true, width: '', formatter: '' },
  ])
