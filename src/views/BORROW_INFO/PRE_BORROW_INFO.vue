<template>
    <div class="container-part">
        <el-form id="form1" class="form-part" :inline="true" :model="formData1">
                            <el-form-item label="团名" label-width="70">
                       <el-input id="txb_1L__book_name" v-model="formData1.txb_1L__book_name" placeholder="模糊查询" MaxLength="20" clearable></el-input>
                    </el-form-item>
        ...
        <el-table id="table1" class="table-part" :data="tableData1" border>
            ...
            <template v-for="col in colList">
                <el-table-column :prop="col.prop" :label="col.label" :width="col.width" v-if="col.isShow" :formatter=" col.formatter==''? false : formatCellDate"></el-table-column>
            </template>
            <!-- 操作按钮列↓ -->
...
const showCols = ref([
'seq','book_name','reader_name','ins_time','borrow_days','borrow_price','borrow_cost','remark','book_id','reader_id',
])

//表格列  展示之前按no排序;
const colList = ref([
      { no: 0, prop: 'seq', label: 'seq', isShow: true, width: '', formatter: '',  },
      { no: 0, prop: 'book_name', label: '团名', isShow: true, width: '', formatter: '',  },
      { no: 0, prop: 'reader_name', label: '读者姓名', isShow: true, width: '', formatter: '',  },
      { no: 0, prop: 'ins_time', label: '添加时间', isShow: true, width: '', formatter: 'formatCellDate',  },
      { no: 0, prop: 'borrow_days', label: '计划借阅时长(天)', isShow: true, width: '', formatter: '',  },
      { no: 0, prop: 'borrow_price', label: '借阅单价(元/天)', isShow: true, width: '', formatter: '',  },
      { no: 0, prop: 'borrow_cost', label: '借阅费用', isShow: true, width: '', formatter: '',  },
      { no: 0, prop: 'remark', label: '备注', isShow: true, width: '', formatter: '',  },
      { no: 0, prop: 'book_id', label: '书ID', isShow: true, width: '', formatter: '',  },
      { no: 0, prop: 'reader_id', label: '读者ID', isShow: true, width: '', formatter: '',  },

])

//配置显示列选择器
const showColoptions = [
      { no: 0, value: 'seq', label: 'seq',  },
      { no: 0, value: 'book_name', label: '团名',  },
      { no: 0, value: 'reader_name', label: '读者姓名',  },
      { no: 0, value: 'ins_time', label: '添加时间',  },
      { no: 0, value: 'borrow_days', label: '计划借阅时长(天)',  },
      { no: 0, value: 'borrow_price', label: '借阅单价(元/天)',  },
      { no: 0, value: 'borrow_cost', label: '借阅费用',  },
      { no: 0, value: 'remark', label: '备注',  },
      { no: 0, value: 'book_id', label: '书ID',  },
      { no: 0, value: 'reader_id', label: '读者ID',  },
